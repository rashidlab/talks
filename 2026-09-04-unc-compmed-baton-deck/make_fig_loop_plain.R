...
p <- ggplot() +
  ...
  geom_tile(data = nodes, aes(x, y), width = 3.0, height = 1.2,
            fill = fog, colour = navy, linewidth = 0.6) +
  geom_text(data = nodes, aes(x, y + 0.06, label = main), size = 5.8,
  ...
  xlim(-0.2, 10.2) + ylim(0.4, 5.5) + theme_void() +
