...
::: {.incremental}
- **MTD-only dose-finding was built for single-agent cytotoxics** [@letourneau2009] — 3+3 is the canonical example; CRM, BLRM, mTPI, BOIN all share the paradigm. Assumptions:
...
These methods predate Optimus by 5–15 years. Optimus made what they deliver the regulatory standard [@fda_optimus_guidance].
:::

::: {style="margin-top: 0.5em; font-size: 0.72em; color: #5B6670; font-style: italic;"}
Implementations: **BOIN family** for printed decision tables [@liu2015boin; @zhou2019uboin]; **POCRM, BLRM** for combinations [@wages2011pocrm; @neuenschwander2008blrm]; **TITE-BOIN** for late-onset toxicity [@yuan2018titeboin].
:::
:::
...
#| fig.width: 5.0
#| fig.align: center
#| fig.alt: "U-BOIN utility surface: toxicity probability (x-axis) vs efficacy probability (y-axis). Red dashed lines mark admissibility cutoffs (max acceptable toxicity 25%, min required efficacy 20%). Six dose levels DL1-DL6 plotted as points; admissible doses (inside both cutoffs) are navy, inadmissible doses are gray, and the OBD (admissible dose with highest utility) is a red diamond."
library(ggplot2)

# Utility surface
tox_seq <- seq(0, 0.45, length = 60)
eff_seq <- seq(0, 0.55, length = 60)
util_grid <- expand.grid(tox = tox_seq, eff = eff_seq)
util_grid$utility <- pmax(0, util_grid$eff - 0.5 * util_grid$tox)

# Candidate dose levels
doses <- data.frame(
  dl  = c("DL1", "DL2", "DL3", "DL4", "DL5", "DL6"),
  tox = c(0.05, 0.10, 0.18, 0.27, 0.35, 0.42),
  eff = c(0.10, 0.22, 0.35, 0.42, 0.40, 0.32)
)
tox_max <- 0.25
eff_min <- 0.20
doses$admissible <- doses$tox <= tox_max & doses$eff >= eff_min
doses$utility    <- doses$eff - 0.5 * doses$tox
doses$is_obd     <- FALSE
doses$is_obd[ which(doses$admissible)[which.max(doses$utility[doses$admissible])] ] <- TRUE

ggplot() +
  geom_tile(data = util_grid, aes(x = tox, y = eff, fill = utility), alpha = 0.85) +
  ...
  geom_hline(yintercept = eff_min, color = "#C75050",
             linewidth = 1.1, linetype = "dashed") +
  annotate("text", x = tox_max + 0.008, y = 0.55,
           label = "Max acceptable\ntoxicity (25%)",
           color = "#C75050", hjust = 0, fontface = "bold",
           size = 3.2, lineheight = 0.9) +
  annotate("text", x = 0.44, y = eff_min - 0.025,
           label = "Min required efficacy (20%)",
           color = "#C75050", hjust = 1, fontface = "bold", size = 3.2) +
  ...
  geom_point(data = doses[doses$is_obd, ],
             aes(x = tox, y = eff),
             shape = 18, color = "#C75050", size = 13) +
  geom_text(data = doses, aes(x = tox, y = eff, label = dl),
            color = "white", size = 2.8, fontface = "bold") +
  annotate("text",
           x = doses$tox[doses$is_obd] + 0.025,
           y = doses$eff[doses$is_obd] + 0.035,
           label = "OBD", color = "#C75050",
           size = 5.4, fontface = "bold", hjust = 0) +
  ...
  theme_minimal(base_size = 11) +
  theme(panel.grid = element_blank(),
        axis.title = element_text(face = "bold", color = "#13294B"))
```

::: {style="margin-top: 0.2em; font-size: 0.7em; color: #5B6670; font-style: italic; text-align: center; line-height: 1.3;"}
Each dose-level plots on the toxicity × efficacy plane. Admissible doses (inside the red dashed lines) are evaluated for utility; the **OBD** is the admissible dose with the highest utility.
:::
:::
