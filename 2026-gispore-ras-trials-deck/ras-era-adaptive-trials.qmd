...
The statistical methods exist to **optimize combination doses**.
:::

::: {.fragment style="margin-top: 0.4em; color: #13294B;"}
The statistical methods exist to **adapt trials to emerging resistance**.
:::
...
ggplot(df, aes(x = x, y = y, color = cls)) +
  ...
  coord_fixed() +
  scale_x_continuous("Calibration parameter A (e.g., futility threshold)",
                     limits = c(0, 1), expand = c(0.01, 0.01)) +
  scale_y_continuous("Calibration parameter B (e.g., max N)",
                     limits = c(0, 1), expand = c(0.01, 0.01)) +
  theme_minimal(base_size = 11) +
...
p_baton <- ggplot() +
  ...
  labs(title = "BATON search",
       subtitle = "~30 designs, model-guided; focuses on feasible") +
  theme_minimal(base_size = 11) +
  theme(panel.grid = element_blank(),
        plot.title = element_text(hjust = 0.5, face = "bold",
