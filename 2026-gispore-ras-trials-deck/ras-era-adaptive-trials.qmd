...
*Joint surface = the two-drug benefit-risk landscape — how efficacy and toxicity respond at every (A, B) dose pair, not just along one drug's axis.*
:::
...
**Design:** *Stage 1* — establish the safety envelope [@liu2015boin]. *Stage 2* — find the dose with the best response/toxicity trade-off [@zhou2019uboin]. **Up to 52 patients; ~28 expected at the OBD.**
:::
...
*Calibration tunes a design to hit its **operating characteristics** — the trial's report card: how often it catches real drug effects and rejects non-working drugs, how many patients enrolled, how long it runs.*
:::
...
## BATON delivers all four goals — for PANGEA Phase II {.smaller}

```{r}
#| fig.height: 4.4
#| fig.width: 10
#| fig.align: center
#| fig.alt: "Four-box layout echoing the calibration-challenge slide, now showing BATON's achieved values for PANGEA Phase II. Box 1: FDA compliance checkmark, Type I 7.0% (target ≤10%) and Power 82.2% (target ≥80%). Box 2: Minimize patients checkmark, ~23 spared, avg N 53 vs Max N 76. Box 3: Stop bad early checkmark, 78% futility stop when drug inactive. Box 4: Accelerate good checkmark, 81% efficacy stop when drug active."
library(ggplot2)
...
**70% fewer evaluations than grid search** — same optimum. Manual calibration: weeks to months. BATON: hours, with an FDA-grade audit trail.
:::

::: {.notes}
This is the payoff slide for the BATON pitch — and the structural mirror of slide 20. Slide 20 named four competing goals every adaptive trial must satisfy simultaneously; this slide shows BATON delivering on all four for PANGEA Phase II. Read out loud, in order: (1) FDA compliance — Type I 7.0%, Power 82.2%, both inside the target boundaries. (2) Minimize patients — about 23 patients spared on average (expected N of 53 against a max of 76) vs a fixed-N trial. (3) Stop bad early — when the drug is inactive, the trial stops early for futility 78% of the time, so patients aren't randomized to a losing arm waiting for the readout. (4) Accelerate good — when the drug works, the trial stops early for efficacy 81% of the time, so the answer reaches the field faster. The heatmap on the bottom is the mechanism — grid search spreads 100 evaluations across a 10×10 lattice; BATON converges on the same optimum in 30 sequential evaluations because each one informs the next. 70% fewer evaluations is the difference between hours and weeks given each evaluation is thousands of Monte Carlo trial replicates. This is a 2D illustration; PANGEA Phase II's design space is 4-dimensional and EVOLVE designs run 8–13 dimensions, where the efficiency advantage compounds. CREDIT DISCIPLINE — the four achievements are properties of the calibrated adaptive design class; BATON's contribution is finding the parameter configuration that delivers them. Say "the BATON-calibrated design delivers..." not "BATON delivers..." — credit the design, not the optimizer. PRIOR STRUCTURE — this used to be two slides: a Grid-vs-BATON heatmap with a manual-vs-BATON comparison table, then a separate "patients spared" slide with three big-number callouts. Merged because the two slides were proving the same thesis; this version ties the proof back to slide 20's four-goal framing and demotes the search-mechanism heatmap to a supporting strip. If a methodologist asks for the convergence picture or the manual-vs-BATON operational comparison, point them to the BATON manuscript (Young et al., 2026).
:::
