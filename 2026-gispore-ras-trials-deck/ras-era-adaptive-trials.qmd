...
::: {.column width="50%"}
::: {style="margin-top: 0.2em; font-size: 1.0em;"}
**Phase I no longer ends with a single MTD — it ends with an OBD.**
:::

::: {style="margin-top: 0.5em; color: #5B6670; font-style: italic; font-size: 0.88em;"}
The Bayesian model-assisted toolkit replaces 3+3:
:::

::: {.incremental style="margin-top: 0.4em; font-size: 0.86em; line-height: 1.5;"}
- **BOIN family** [@liu2015boin] — simple as 3+3 (printed decision table), statistically matches model-based designs. Extensions: combinations (**BOIN-COMB** [@lin2017boincomb]), late-onset toxicity (**TITE-BOIN** [@yuan2018titeboin]), benefit-risk (**BOIN12** [@lin2020boin12]). **POCRM** [@wages2011pocrm] and **BLRM** [@neuenschwander2008blrm] are common combination alternatives.
- **U-BOIN** [@zhou2019uboin] — *"a response is worth this; a Grade 3 toxicity costs this."* Stage 1 BOIN safety envelope, Stage 2 utility-guided OBD allocation.
:::
...
::: {style="margin-top: 0.4em; font-size: 0.76em; color: #5B6670; font-style: italic;"}
RAS adoption: sotorasib BLRM [@hong2020sotorasib], adagrasib mTPI in KRYSTAL-1 [@ou2022adagrasib].
:::
:::
...
::: {.slide-refs}
Liu & Yuan 2015 (BOIN) · Zhou, Lee & Yuan 2019 (U-BOIN) · FDA Optimus Guidance · Wages 2011 (POCRM) · Neuenschwander 2008 (BLRM)
:::

::: {.notes}
The toolkit that replaces 3+3 — and the paradigm shift that goes with it. Two reasons Bayesian and model-assisted designs fit modern Phase I: they handle small sequential sample sizes by cohort-by-cohort posterior updates, and they support utility-based dosing — which is how the OBD paradigm becomes operational. BOIN, from Liu and Yuan 2015, is the workhorse: operationally as simple as 3+3 with a printed decision table at the bedside, but statistically matches the best fully model-based designs. The family extends naturally: BOIN-COMB for combinations, TITE-BOIN for late-onset toxicity, BOIN12 for joint toxicity-efficacy. POCRM (Wages and Conaway) and BLRM are the common combination alternatives. U-BOIN is the cleanest example of operationalizing the OBD paradigm. Before the trial starts, your team scores the trade-off: how do you weigh a response against a Grade 3 toxicity? Those scores become the navigation system. Stage one uses BOIN rules to establish the safe envelope. Stage two allocates patients within that envelope to maximize the benefit-risk utility you specified. What comes out of Phase I is not an MTD — it's an OBD, a dose with a characterized benefit-risk profile. That's the dose you carry into Phase II combinations. That's what Project Optimus is asking sponsors to deliver. Credibility line: leading RAS Phase I programs already run on Bayesian designs — sotorasib used BLRM in CodeBreaK 100, adagrasib used mTPI in KRYSTAL-1. The shift Project Optimus formalized is already industry standard for RAS monotherapy. For a GI clinical audience, the key message is one sentence: modern Phase I optimizes benefit-risk instead of maximizing toxicity. The acronym soup is secondary.
:::
...
- Pathway reactivation and downstream amplification: KRAS amp, NRAS / BRAF / MAP2K1, MYC, PI3K family [@aguirre2024resistance; @awad2021resistance; @wasko2024rmc7977]
- RTK upregulation and bypass signaling [@zhuang2026rtk]
- Cell-state plasticity (basal ↔ classical, partial EMT) [@aguirre2024resistance; @xu2025kinome]
:::
:::
...
::: {.incremental style="margin-top: 0.5em; font-size: 0.95em; line-height: 1.55;"}
- **Fixed regimens miss the interception window.** By the time the trial reads out, the biology has changed.
- **Baseline-only biomarkers can't track evolution under therapy.** You measured the wrong thing at the wrong time.
- **Resistance is characterized retrospectively** — only after progression and trial closure [@awad2021resistance; @aguirre2024resistance]. The field's best resistance knowledge was generated exactly the way I'm criticizing.
- **The temporal surface** — when resistance emerges, in which patients, by which mechanism — **is never characterized.**
:::
...
**Genomic → ctDNA · non-genomic / plasticity → biopsies, transcriptomics, imaging — a resistance-aware trial has to *do both* during enrollment.** Same structural failure as combination dose-finding: static designs for a dynamic problem.
:::
...
::: {.column width="50%"}
::: {style="margin-top: 0.4em; font-size: 0.82em; line-height: 1.45;"}
**EVOLVE-BDT (real, enrolling)** — 2L MBC, ~700 pts

- ER+/HER2− and TNBC subtrials
- Subgroups A–D by **ESR1**, **PI3K/AKT/PTEN**, **PD-L1**, **AR** status
- Serial tumor + blood at every visit
- Arms swap on Bayesian futility/efficacy
:::
:::

::: {.column width="50%"}
::: {style="margin-top: 0.4em; font-size: 0.82em; line-height: 1.45;"}
**PDAC analog (hypothetical)** — 2L mPDAC on daraxonrasib + GnP

- **Basal-like vs classical by PurIST** [@purist_rashid2020]
- Subgroups by **KRAS amp / RTK upregulation / pEMT** [@aronchik2025ena]
- Serial **ctDNA + biopsies** at every visit
- Arms: + EGFR-i, + SHP2-i, classical-state shifters
:::
:::

::::

::: {.fragment style="margin-top: 0.4em; padding: 0.5em 1em; background-color: #F8F9FA; border-left: 4px solid #4B9CD3; color: #13294B; font-size: 0.95em; text-align: center;"}
**Same architecture, different disease.** Every row of the right column has a real biomarker, a real assay, and a real combination partner already in or entering the clinic.
:::
...
**Continual learning during enrollment is not a tweak to a Phase II — it is a structurally different trial.** Each axis adds interacting parameters: interim timing, posterior-probability thresholds, biomarker decision rules, randomization-weight updates, cohort-opening logic. **The Bayesian adaptive methodology exists. Making it deployable — calibrating it to the operating-characteristic targets the team and the FDA agree on — is the gap.**
:::
