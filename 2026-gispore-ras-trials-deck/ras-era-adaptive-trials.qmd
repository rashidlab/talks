...
What's missing is making those designs practical to calibrate and launch.
:::
...
- **3+3 was built for single-agent chemo** [@letourneau2009]. It assumes monotonic dose-toxicity, additive joint toxicity, Cycle-1 DLTs, and one-drug-at-a-time titration. **Combinations violate all four.**
- **In practice, combination dose-finding collapses to one drug.** Pick agent A's dose, fix it, add agent B — the dominant pattern. Even when both drugs *are* dose-escalated, designs treat the combinations as independent: in Rivière's review of 162 such trials, **88% used 3+3 designs developed for single-agent evaluation** — no joint-surface modeling [@riviere2015combo; @harrington2013dual].
...
- **Severe *immune-mediated* hepatotoxicity, not predictable from the single agent** — sotorasib's pro-inflammatory TME effect potentiates pembro's latent T-cell response
- **88% of Grade 3-4 events fell outside the Cycle 1 DLT window** [@codebreak_io2022] — ICI-toxicity kinetics, not cytotoxic kinetics; same mechanism, second visible failure
:::
...
**Three of the four 3+3 assumptions fail at once:** non-additive toxicity, late-onset toxicity, one-drug-at-a-time titration.
:::
...
::: {.fragment style="margin-top: 1.0em; padding: 0.6em 1em; background-color: #F8F9FA; border-left: 4px solid #4B9CD3; font-size: 0.92em;"}
This is no longer a methodological preference. It's the regulatory baseline.
:::

::: {.fragment style="margin-top: 0.5em; padding: 0.5em 0.9em; background-color: #F8F9FA; border-left: 3px solid #4B9CD3; font-size: 0.82em; color: #13294B;"}
...
::: {style="margin-top: 0.3em; font-size: 0.95em; text-align: center;"}
**Four design axes — disease-agnostic.** A RAS-backbone PDAC combination would need all four.
:::
...
::: {.fragment style="margin-top: 0.6em; padding: 0.8em 1em; background-color: #13294B; color: #FFFFFF; font-size: 1.05em; text-align: center; font-weight: 600;"}
Methods exist. **We're closing the deployment gap now.**
:::
...
ggplot(steps, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = detail, x = 1.62), size = 3.8, color = "#444444",
...
- Four interacting parameters rarely hit both constraints simultaneously
...
- Four parameters jointly calibrated; both constraints met
...
**BATON surfaced this during PANGEA Phase II calibration:** caps set too low suppress early-efficacy declarations. Manual calibration anchors on the first feasible design; BATON returned the Pareto frontier.
:::
:::
