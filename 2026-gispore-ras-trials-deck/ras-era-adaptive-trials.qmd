---
title: "Adaptive Trial Design for the RAS-Inhibitor Era: Combinations, Resistance, and Biomarker-Guided Therapy in GI Oncology"
...
::: {style="margin-top: 0.6em; text-align: center; font-size: 1.0em; color: #13294B;"}
**The calibrated design for PANGEA Phase II:** meets Type I ≤ 10% and Power ≥ 80%, and spares ~23 patients vs a fixed-N trial.
:::
...
ggplot(callouts, aes(x = x)) +
  ...
  geom_text(aes(y = 0.30, label = label),
            size = 5, color = "#333333", lineheight = 0.95) +
  scale_color_identity() +
  scale_x_continuous(limits = c(0.4, 4.4), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0.10, 0.95), expand = c(0, 0)) +
  theme_void()
```

::: {style="margin-top: 0.4em; text-align: center; font-size: 0.85em; color: #5B6670; font-style: italic;"}
Savings come from adaptive stopping; BATON's job is finding the design configuration that delivers them under the OC constraints.
:::

::: {.notes}
Same BATON story as the grid-vs-BATON slide, but reframed in clinically relevant units. Three numbers carry the slide: ~23 patients spared relative to a fixed-N=76 design, Type I error 7.0% (target ≤10%), Power 82.2% (target ≥80%). CREDIT DISCIPLINE — the patients are spared by the adaptive design class itself; early futility/efficacy stopping is what makes average N of 53 possible against a max of 76. BATON's contribution is finding the configuration of stopping thresholds and futility margin that delivers those operating characteristics. Say "the BATON-calibrated adaptive design spares ~23 patients" or "the design BATON found spares ~23 patients" — not "BATON spares ~23 patients" — the second sentence credits the optimizer rather than the design. The achieved expected N of 53 against the calibrated max of 76 is what you'll see on the OC table slide after the vignette. PRIOR VERSION had a convergence plot showing each of ~100 candidate designs BATON evaluated, color-coded by whether they met the FDA OC constraints. Removed for accessibility — the curve was statistician-grade and the audience reads the three big numbers anyway. If a methodologist asks for the convergence picture, point them to the BATON manuscript (Young et al., 2026).
:::
